 ...
 *
 */
@Component("hotelOrderItemBussiness")
public class HotelOrderItemBussiness  extends AbstractBookService implements OrderInitBussiness{
...
//		BuyInfo buyInfo = order.getBuyInfo();
	...
	private String fillOrderItemContentWithHotelAdditationInfo(OrdOrderItem orderItem, Item item, OrdOrderDTO ordOrderDTO,SuppGoods suppGoods) {
		...
		if ((item != null) && (item.getHotelAdditation() != null)) {
			...
			if (errorMsg == null) {
				orderItem.putContent(OrderEnum.HOTEL_CONTENT.latestLeaveTime.name(), item.getHotelAdditation().getLatestLeaveTime());
		...
		if (hotelAdditation != null) {
			...
			if (hotelAdditation.getArrivalTime() != null) {
				if (hotelAdditation.getEarlyArrivalTime() == null) {
					if (propMap.get("earliest_arrive_time") != null) {
			...
			//设置最晚离店时间
			if (hotelAdditation.getLatestLeaveTime() == null) {
				if(propMap.get("latest_leave_time") != null){
				    String latestLeaveTime = propMap.get("latest_leave_time").toString();
					if (latestLeaveTime.indexOf(":") < 0) {
						latestLeaveTime = latestLeaveTime + ":00";
					}
					
					if (!latestLeaveTime.matches(timeReg)) {
						errorMsg = "酒店入住的最晚离店时间不正确，latestLeaveTime=" + latestLeaveTime;
						LOG.debug("method checkHotelAdditationInfo: msg=" + errorMsg);
					}
					hotelAdditation.setLatestLeaveTime(latestLeaveTime);
			  }
	...
	private static final Logger LOG = LoggerFactory.getLogger(HotelOrderItemBussiness.class);
}
