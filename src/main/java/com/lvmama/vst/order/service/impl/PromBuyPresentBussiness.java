...
@Component
public class PromBuyPresentBussiness {
	private static Logger logger = LoggerFactory.getLogger(PromBuyPresentBussiness.class);
	@Autowired
	 ...
	 * @return
	 */
	public BuyPresentActivityInfo findPromBuyPresentForOrder(OrdOrderDTO order){
		try {
			...
			if(packList!=null&&!packList.isEmpty()){
				...
				if(categoryId!=null && Constant.VST_CATEGORY.CATEGORY_SINGLE_TICKET.getCategoryId().equals(categoryId.toString())){
					...
					for(OrdOrderItem item:itemList){
						...
						result= buyPresentClientService.gainToOrderActivityListIdsForSaveOrder(paramsList, userId);
					}
				}
				else{
	 ...
	 * @return
	 */
	public BuyPresentActivityInfo findPromBuyPresentForOrder(OrdOrderDTO order,Long userId){
		try {
			List<OrdOrderPack> packList = order.getOrderPackList();
			long quantity = 0;
		//	Long userId = order.getBuyInfo().getUserNo();
