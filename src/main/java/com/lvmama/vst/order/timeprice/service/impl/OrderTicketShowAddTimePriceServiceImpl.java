 ...
 *
 */
@Component("orderTicketShowAddTimePriceService")
public class OrderTicketShowAddTimePriceServiceImpl extends AbstractOrderTimePriceService{
	/**
	 * Logger for this class
	 */
	private static final Logger logger = LoggerFactory.getLogger(OrderTicketShowAddTimePriceServiceImpl.class);

	@Autowired
	private DistGoodsTimePriceClientService distGoodsTimePriceClientService;
	
	@Resource(name="goodsOraTicketShowAddTimePriceStockService")
	private IGoodsTimePriceStockService goodsTicketShowAddTimePriceStockService;
	@Autowired
	...
	public ResultHandleT<Object> checkStock(SuppGoods suppGoods, Item item,
		...
		try{
			...
			ShowGoodsAddTimePrice addTimePrice = (ShowGoodsAddTimePrice)timePrice;
...
//		if(!goodsTicketAddTimePriceStockService.updateStock(timePriceId, -stock)){
		try {
			if(!goodsTicketShowAddTimePriceStockService.updateStock(timePriceId, -stock, dataMap)){
	...
	private void fillPrice(OrdOrderItem item,ShowGoodsAddTimePrice timePrice){
	 ...
	 * @param timePrice
	 */
	private void fillPrice_autoPack( OrdOrderItem item,ShowGoodsAddTimePrice timePrice){
		item.setPrice(timePrice.getPrice());
		item.setSettlementPrice(timePrice.getSettlementPrice());
		item.setMarketPrice(timePrice.getMarkerPrice());
...
//		item.setPrice(item.getPrice());
	...
	public ResultHandleT<SuppGoodsBaseTimePrice> getTimePrice(Long goodsId,
		...
		SuppGoodsBaseTimePrice timePrice = goodsTicketShowAddTimePriceStockService.getTimePrice(goodsId, specDate, checkAhead);
	...
	public void updateRevertStock(Long suppGoodsId, Date specDate, Long stock,
			Map<String, Object> dataMap) {
		SuppGoodsBaseTimePrice timePrice = goodsTicketShowAddTimePriceStockService.getTimePrice(suppGoodsId, specDate, false);
		if(timePrice!=null){
			try {
				goodsTicketShowAddTimePriceStockService.updateStock(timePrice.getTimePriceId(), stock, dataMap);
			} catch (Exception e) {
