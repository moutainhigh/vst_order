...
import com.lvmama.vst.order.dao.goods.SuppGoodsSingleTimePriceOraDao;
 ...
 *
 */
@Service("goodsOraSingleTimePriceStockService")
public class GoodsOraSingleTimePriceStockServiceImpl implements IGoodsTimePriceStockService {
	
	@Autowired
	private SuppGoodsSingleTimePriceOraDao suppGoodsSingleTimePriceDao;
	...
	public boolean updateStock(Long timePriceId, Long stock) {
		...
		if(timePriceId != null) {
			...
			int ret = suppGoodsSingleTimePriceDao.updateStockForOrder(params);
			
			if (ret > 0) {
				SuppGoodsSingleTimePrice timePrice = suppGoodsSingleTimePriceDao.selectByPrimaryKey(timePriceId);
//				comPushServiceRemote.pushTimePrice(timePrice.getSuppGoodsId(), Collections.singletonList(timePrice.getSpecDate()), ComIncreament.DATA_SOURCE_TYPE.ORDER_STATUS);
	...
	public SuppGoodsBaseTimePrice getTimePrice(Long goodsId, Date specDate, boolean checkAhead) {
		return suppGoodsSingleTimePriceDao.getTimePrice(goodsId, specDate, checkAhead);
	}
