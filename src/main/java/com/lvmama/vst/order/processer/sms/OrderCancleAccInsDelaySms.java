...
/** 
 * @Title: OrderCancleDelayAccInsSms.java 
 * @Package com.lvmama.vst.order.processer.sms 
 * @Description: TODO 
 * @author wangsizhi
 * @date 2017-1-23 下午4:13:48 
 * @version V1.0.0 
 */
public class OrderCancleAccInsDelaySms implements AbstractSms {

    /**
     * Logger for this class
     */
    private static final Logger logger = LoggerFactory.getLogger(OrderCancleAccInsDelaySms.class);
    
    private OrdAccInsDelayInfo ordAccInsDelayInfo;
    
    public OrderCancleAccInsDelaySms(OrdAccInsDelayInfo ordAccInsDelayInfo) {
    ...
    public List<String> exeSmsRule(OrdOrder order) {
     Long orderId = order.getOrderId();
        ...
        logger.info("OrderCancelSms ===>>> CancleDelayAccIns(order)=" + order.getOrderId()); 
        //发送规则列表
        ...
        if(isAccInsDelay(order)&&isPayed(order)&&orderStatus(order)){
            sendList.add(OrdSmsTemplate.SEND_NODE.CANCEL_ACCINS_DELAY.name());
        }
       else{
            if (logger.isWarnEnabled()) {
                logger.warn("exeSmsRule(OrdOrder) - don't found CancleDelayAccIns template"); //$NON-NLS-1$
            }               
        }
    ...
    public boolean isAccInsDelay(OrdOrder order){
        ...
        if (StringUtils.isNotBlank(travDelayFlag) && StringUtils.equalsIgnoreCase(travDelayFlag, "Y")
                ...
                && StringUtils.equalsIgnoreCase(travDelayStatus,  OrderEnum.ORDER_TRAV_DELAY_STATUS.ABANDON.name())) {
            return true;
        }
