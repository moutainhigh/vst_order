 ...
 *
 */
public class ComJobConfigExecutorJob implements Runnable {
	private static final Log LOG = LogFactory.getLog(ComJobConfigExecutorJob.class);
	...
	@Override
	public void run() {
		if(Constant.getInstance().isJobRunnable() && Constant.getInstance().isComExecutorJobRunnable()) {
			List<ComJobConfig> comJobConfigList = comJobConfigService.selectList(null, new Date());
			if (comJobConfigList != null && comJobConfigList.size() > 0) {
				...
				for (ComJobConfig comJobConfig : comJobConfigList) {
					if (comJobConfig != null) {
						LOG.info("ComJobConfigExecutorJob.run:comJobConfig(ID=" + comJobConfig.getComJobConfigId() + ",ObjectId=" + comJobConfig.getObjectId() + ",JobType=" + comJobConfig.getJobType());
					}
					executor = comJobConfigExecutorFactory.createComJobConfigExecutor(comJobConfig);
					if (executor != null) {
						
						ResultHandleTT<ComJobConfig> result= executor.execute(comJobConfig);
						if(comJobConfig != null) {
							LOG.info("ComJobConfigExecutorJob.run::comJobConfig(ID=" + comJobConfig.getComJobConfigId() + "),code=" + code);
						}
						code = result.getCode();
...
//						}
						...
						LOG.info("ComJobConfigExecutorJob.pass: not my job");
					}
				}
			}
		}
