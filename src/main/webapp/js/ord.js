...
//显示弹出层
function showDetailDivByAmount(divName, amount,orderId,sysCode) {
	document.getElementById(divName).style.display = "block";
	document.getElementById("bg").style.display = "block";
	//请求数据,重新载入层
	$("#" + divName).reload({"amount":amount,"orderId":orderId,"sysCode":sysCode});
}
//ajax发送请求操作
...
function doCancelOrder(divName) {
	...
	httpRequest("/super_back/ord/doOrderCancel.do", {"orderId":orderId, "orderDetail.cancelReason":reason}, true, divName);
}
//需重播
