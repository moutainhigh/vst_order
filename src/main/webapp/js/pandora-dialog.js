(function (global, $, undefined) {
    ...
    Dialog.prototype = {
        ...
        _createIframe: function (url) {
            var that = this;
            var wrap = that.wrap;
            var $dialogBody = wrap.find('.dialog-body');
            ...
            this.iframe.one("load", function () {
                ...
                var thatH = that.config.height;
                if (thatH !== "" && thatH !== "auto") {
                    wrap.css("height", thatH);
                    $dialogBody.height(thatH - wrap.find('.dialog-header').outerHeight()-wrap.find('.dialog-footer').outerHeight() -($dialogBody.outerHeight()-$dialogBody.height()) );
                }else{
        ...
        _syncWH: function () {
            ...
            var thatH = this.config.height;
            if (thatH !== "" && thatH !== "auto") {
                wrap.css("height", thatH);
                this._bodyWH();
                //wrap.find("div.dialog-content").css({ "height": this.config.height });
            }else{
                // 去除dialog 固定高度
                wrap.css("height", "auto");
                wrap.find("div.dialog-content").css({ "height": h +20 });
