...
(function($) {
	...
	$.fn.jsonSuggest = function(settings) {
		var defaults = {  
				...
				wildCard: '',
				emptyKeyup:true,//是否需要针对文本框的值变成没有值时做处理.默认是要处理.
				caseSensitive: false,
				notCharacter: '!',
				maxHeight: 350,
				useGlobalClear: false,//隐藏标记，默认点页面其他位置不隐藏
		...
		return this.each(function() {
			...
			*/
			...
			//添加搜索背景.
			obj.addClass("searchInput");
			if(obj.val()!=''){
			    obj.data("selectValue",obj.val());
			}
			/**
			...
			*/
			function selectResultItem(item) {
				obj.val(item.text);
				$(results).html('').hide();
				obj.data("selectValue",item.text);	
			...
			*/
			function runSuggest(e) {	
				...
				//修改：如果没有URL才走Data
				if (!(settings.url && typeof settings.url === 'string') && settings.data && settings.data.length) {
					...
					if(settings.param){
						for(var i=0;i<settings.param.length;i++){
							var $tag=$(settings.param[i]);
							var key=$tag.attr("name");
							var v;
							var tagName=$tag.get(0).tagName.toUpperCase();
							if(tagName=='SELECT'){
								v=$tag.find(":selected").val();				
							}else if(tagName=='INPUT'){
								if($tag.attr("type").toUpperCase()=='RADIO'||$tag.attr("type").toUpperCase()=='CHECKBOX'){
									v=$tag.find(":checked").val();	
								}else{
									v=$tag.val();
								}
							}
					...
					$(results).html('<li class="ui-menu-item ajaxSearching"><a class="ui-corner-all">查找中...</a></li>').
						show().css('height', 'auto');
					
					getJSONTimeout = window.clearTimeout(getJSONTimeout);
					$.extend(formData,{search: text});
					getJSONTimeout = window.setTimeout(function() {
						$.post(settings.url, formData, function(data) {
							//var data=eval("("+dt+")");
							try{
								if (data) {
			...
			*/
			function keyListener(e) {
				//对空值时做处理,此处修改过.
				if($.trim($(this).val())==''){
					if (settings.emptyKeyup &&typeof settings.onSelect === 'function') {
						var item={id:""};
						settings.onSelect(item);
					}
				}
			...
			obj.click(function(e){
				...
				if (val=='' && settings.data && settings.data.length) {
					buildResults(settings.data,'');
				}
			})
			...
			obj.after(results).
				...
				change(function(e){
					var val=$.trim($(this).val());
				     ...
				     if(val==''||typeof(selectValue)==undefined||val!=selectValue){
				      if (settings.emptyKeyup && typeof settings.onSelect === 'function') {
				       var item={id:""};
				       settings.onSelect(item);
				      }
			...
			if(settings.useGlobalClear){
				$(document).click(function(e){
					var $clickObj=$(e.srcElement);
					//alert($clickObj.parent().html())
					//alert($(obj).parent().html())
					//alert(($clickObj[0]===obj[0])+"    "+($clickObj[0]==$(results)[0]));
					if($clickObj!=null&&$clickObj[0]==$(obj)[0]||$clickObj[0]==$(results)[0]){
						;
					}else{
						$(results).hide();
					}
