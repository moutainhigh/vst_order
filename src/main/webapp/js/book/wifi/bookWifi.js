...
function souceFnCallBack(cal){
	pandora.calendar.mos = 12;
	    ...
	    var minDay = $(cal.options.trigger).attr("minDay");
	    if(productType == "WIFI"){
	    	url = "/vst_order/ord/book/wifi/getWifiTimePrice.do";
	   	 if (cal.$trigger[0] === $(cal.options.trigger).eq(0)[0]){
	   		 	endFlag = "N";
			 }else if(cal.$trigger[1] === $(cal.options.trigger).eq(0)[1]){
				 endFlag = "Y";
			 }
	   	 	calTimeprice(cal,url,suppGoodsId,visitTime,endFlag);
	    	
	    }else if(productType == "PHONE"){
	    	url = "/vst_order/ord/book/wifi/getPhoneCardTimePrice.do";
	    	endFlag = 'N';
	    	calTimeprice(cal,url,suppGoodsId,visitTime,endFlag);
...
function selectDateCallback(cal) {
    ...
    if(productType == "WIFI"){
        ...
        offset = $(cal.options.trigger).eq(1).offset();
        if (cal.$trigger[0] === $(cal.options.trigger).eq(1)[0]){
        	 if (day < 0) {
          		$(cal.options.trigger).eq(1).val("");
          		return;
           } 
        	 //$(".ui-calendar").find("td[date-map]").children("div").removeClass("today");
			 //$(".ui-calendar").find("td[date-map='"+date+"']").children("div").addClass("today");
        	 if($("#depositVisitime").size()>0){
        		 $("#depositVisitime").val($(cal.options.trigger).eq(1).val());
        	 }
        	 calendarConfirmCallback.invoke(); 
         }else{
        	 if($(cal.options.trigger).eq(1).val()!=''){
        		 $(cal.options.trigger).eq(1).val("");
        	 }
         }
