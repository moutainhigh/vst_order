<html xmlns="http://www.w3.org/1999/xhtml"> 
 ...
 <form action="#" method="post" id="dataForm">
    ...
    <input type="hidden" name="travellers" value="${travelContractVO.travellers!''}">
    <input type="hidden" name="suitableName" value="${travelContractVO.suitableName!''}">
 ...
 <div class="txt-hetong">
    ...
    <p>
     ...
     3. 委托方在“驴妈妈旅游网”在线订购产品或委托其客服工作人员电话订购的，就本协议所需填写部分可以通过在线填写电子订单或口头委托填写等方式确认协议条款并依 “驴妈妈旅游网”的预订流程体现至在线本协议文本中，对双方均有法律上的强制约束力。<br/>
    ...
    <p>
        ...
        <input type="text" style="width: 160px; margin-right: 10px;" name="" id="" readonly="readonly" value="${travelContractVO.suitableName!''}" />
        <#else>
        <input type="text" style="width: 160px; margin-right: 10px;" name="" id="" readonly="readonly" value="${travelContractVO.travellers!''}" />
        </#if>
        <b>联系电话：</b>
        <input type="text" style="width: 160px; margin-right: 10px;" name="contractMobile" id="contractMobile" value="${(order.contactPerson.mobile)!'/'}"/><br>
        <b>受托方（以下简称乙方）：</b>
        <input type="text" style="width: 160px; margin-right: 10px;" name="filialeName" id="filialeName" value="${travelContractVO.filialeName}"/>
        <b>联系电话：</b><input type="text" style="width: 160px; margin-right: 10px;" name="lvMobile" id="lvMobile" value="${(travelContractVO.lvMobile)!'1010-6060'}"/><br>
        <br/>
        本协议系乙方代为甲方办理门票、往返机票、签证及酒店预订、机场接送等各类委托预订服务，且为了明确双方的权利义务，本着平等协商的原则，就有关事项达成的如下协议：<br/>
        <b>第一条 承诺与保证</b><br/>
        （一）乙方保证是具有国家旅游局认可的具有旅游资质的旅行社。<br/>
        （二）乙方仅为甲方提供本协议约定的服务事项，其他甲方在境内或境外期间自行安排的内容（包括但不限于参加旅游项目、购物、出行等）或尚未安排的内容全部由甲方自行承担费用、责任及风险，与乙方无关。<br/>
        （三）甲方在“驴妈妈旅游网”订购完成后，因自身原因而未能实际实现所委托事项部分或全部时，将自行承担责任。<br/>
        <b>（四）乙方明确告知甲方，无论在境内或境外的自行安排活动期间，请甲方在自己能够控制风险的范围内活动，所产生的责任自负；甲方已明知并愿意自觉遵守。</b><br/>
        （五）甲方就本协议的委托事项已作详尽了解，并自愿接受本协议内容。<br/>
        <b>第二条 申请与交易</b><br/>
        （一）甲方以书面、电话、网络等方式（包括但不限于通过“驴妈妈旅游网”在线或电话预订等方式）向乙方表明委托事项 (以“驴妈妈旅游网”预订流程中的订单信息为准）；甲方提交的其它书面委托事项有同等法律效力。甲方在实际实现了乙方委托事项后（完成订购），乙方按照本协议的受托及服务义务即刻履行完毕。<br/>
        （二）乙方在签订本协议时，对甲方委托事项中涉及的目的地、日期、标准、项目、安全须知等已作如实详细说明和报价，甲方确认已知悉。<br/>
     ...
     <#list travelContractVO.ordTravellerList as ordTraveller>
     <tr>
         <td style="height: 30px;">${ordTraveller.fullName}</td>
         <td><#if ordTraveller.gender == "MAN"> 男<#elseif ordTraveller.gender == "WOMAN">女</#if></td>
         <td>
                    <#if ordTraveller.idType == "ID_CARD">身份证</#if>
					<#if ordTraveller.idType == "ERTONG">儿童无证件</#if>
					<#if ordTraveller.idType == "GANGAO">港澳通行证</#if>
					<#if ordTraveller.idType == "HUIXIANG">回乡证</#if>
					<#if ordTraveller.idType == "HUZHAO">护照</#if>
					<#if ordTraveller.idType == "JUNGUAN">军官证</#if>
					<#if ordTraveller.idType == "TAIBAO">台胞证</#if>
					<#if ordTraveller.idType == "OTHER">其它</#if>
         </td>
         <td>${ordTraveller.idNo!''}</td>
     </tr>
     </#list>
     <#else>
     <tr>
     ...
     <table border="0" cellspacing="1" cellpadding="0" class="tab1">
        ...
        <tr>
        <td colspan="5">总订单支付金额￥${(travelContractVO.traveAmount)!''} （如购买保险则含保险金额，否则不包含保险金额）</td>
        </tr>
     ...
     <b>第四条 双方权利和义务</b><br/>
     （一）甲方在线或电话委托提交订单后，支付全款或预付款时签订本服务协议。甲方对要求乙方办理各类委托预订服务中提供资料及填写的信息等真实性负责。<br/>
     （二）本协议的委托事项以甲方以“驴妈妈旅游网”预订流程中的订单信息为准；双方确认：乙方不安排领队、导游服务（甲方需要陪同服务人员除外）。<br/>
     （三）甲方抵达目的地后将自行保管所有旅行证件；如甲方发生证件遗失，乙方可在能力范围内给予协助；但由此产生本协议以外的费用（包括但不限于补办证件费、交通、延住、机票、陪同或其他服务费等）全部由甲方承担。<br/>
     （四）乙方所代办的服务事项均为甲方直接指定，其代办行为即为甲方的真实意愿表示；甲方如对已代办服务项目有任何其他书面要求，乙方协助甲方联系相关部门。甲方在第三方服务期间遭受人身、财产损失的，乙方可予以协助处理。<br/>
     （五）鉴于乙方仅向甲方提供委托服务，甲、乙双方履行本协议时，除约定由乙方提供的服务内容外，其余部分由甲方自行负责。甲方需了解自身委托事项的具体情况，在签订本协议时或后向乙方索取的其它信息介绍或资料仅作为甲方参考，乙方对参考信息及资料不提供服务，也不承担责任。<br/>
     （六）甲方已知晓：如遇航空公司机票税费、邮轮港务费、签证费等费用调整时，同意乙方按调整后的价格结算。<br/>
     <b>第五条 乙方特别告知甲方：</b><br/>
     （一）如甲方为老人、未成年人、孕妇或有心脏病、高血压、呼吸系统等疾病病史，建议征得医生意见，或经家属同意，或由家属陪伴为妥。甲方在本协议委托事项以外所产生的费用，由甲方自行承担。<br/>
     （二）甲方作为成年人，已明确知晓户外部分活动（包括但不限于潜水、游泳、高速摩托艇、攀岩、蹦极、滑雪、高原旅行等）为高危娱乐，在充分考虑到自身条件后才自愿参加；并自愿承担因参加上述活动而发生任何事故可能造成的任何后果。<br/>
     （三）凡持非中国大陆护照的游客或自备签证的旅游者，应自行办理本次旅行签证和再次回中国大陆的签证（包括港澳、台湾居民往来大陆通行证）。如因签证等问题造成出入境受阻，由甲方自行负责。<br/>
     （四）甲方应遵守社会公共秩序和社会公德，尊重目的地国家（地区）的法律法规、风俗习惯、文化传统和宗教信仰，爱护旅游资源，保护生态环境，遵守文明行为规范，不参与色情、赌博和涉毒等活动。若违反，后果自负。<br/>
     （五）乙方提示甲方购买个人旅游意外保险，甲方已明确知晓。<br/>
     （六）甲方同意，在乙方完成本协议的委托事项义务后，因不可抗力或天气、罢工、飞机、火车及轮船等一切非乙方原因造成甲方行程时间变更的，由甲方自行和服务提供商协商解决，乙方不承担违约或侵权责任。<br/>
     （七）甲方知晓：签证服务是否给予签证或签注，是否准予出入境，是外国政府、使领馆及有关部门的权力，如因甲方的自身原因或因提供材料存在问题及其他不可归责于乙方的原因而不能及时办理签证或签注而影响行程的，以及被有关部门拒发签证或签注或不准出入境的，相关责任和乙方实际已发生的费用（机票、酒店定金等）由甲方承担；乙方不退还相应服务费。<br/>
     （八）乙方将根据甲方的本协议委托事项，对甲方递交的资料进行初步审核；甲方同意并配合乙方增补所需材料和提供必要的保证金。<br/>
     （九）甲方与乙方办理本协议委托事项后，若因甲方自身原因被拒签或拒绝出入境，或在乙方完成本协议委托服务之前甲方自行取消旅游等导致本协议提前解除，或国家旅游局未发出建议中国公民暂缓前往该目的地的旅游警告解除时，由甲方支付乙方实际已发生费用（机票、酒店定金、签证费等） ，以及乙方服务费。<br/>
     （十）甲方有义务按使领馆要求在规定时间、地点前往使领馆进行签证面试，并在回国后必须按使领馆要求履行销签义务，在规定时间内去使领馆面试销签，因此发生的路费和误工费以及产生的其他责任由甲方自理，与乙方无关。<br/>
     <b>（十一）甲方同意：</b>若因非乙方的原因，造成委托事项变更的，以乙方最终确认信息和实际收到的所有票据、凭证为准。<br/>
     （十二）委托过程中，第三方所提供的赠送活动及相关赠品，甲方若不接受，则视为自愿放弃，费用不予另退或抵扣。<br/>
     （十三）如甲方选择了自由组合航班或单订船票，因网上数据更新有一定的延时，甲方所选定的航班、舱位和价格以乙方操作人员回复为准。乙方将根据民航或邮轮公司订座系统实时信息与甲方确认。<br/>
     （十四）甲方已知晓乙方提供的机票必须按顺序使用,否则航空公司有权取消后续行程的机位。<br/>
     （十五）甲方已知晓乙方提供的特价机票或优惠价机票属于不可签转、更改、退票，不可累计里程的，甲方应当对此完全了解，并愿意按照航空公司规定使用。<br/>
     <b>（十六）甲方确认，本人是在签订本协议前已仔细阅读并详细了解了全部条款内容，乙方人员亦向。甲方对本协议内容进行了详细说明后，本人才自愿签订本委托协议。签署本服务协议的甲方代表人，对自身的代理权真实性和合法性承担相应法律责任，并承担负责将本协议内容转告未签名同行者的责任。</b><br/>
     <b>第六条 退改说明</b><br/>
     详见附件 1《补充条款》<br/>
     <b>第七条 其他</b><br/>
     （一）本合同一式两份，双方各持一份，《委托服务协议书使用说明》、补充条款（即退改说明）均有正文条款法律效力，此协议自双方签章且乙方资源审核完毕同时甲方付清全部应付费用后生效。<br/>
     （二）甲乙双方在履行本协议过程中发生争议的，甲乙双方应协商解决，协商不成提交本合同依法向人民法院起诉。<br/><br/>
     <span style="float: right"><b>【正文结束】</b></span><br/>
     <br/>
     <br/>
    </p>
     <p style="position: relative; padding-bottom: 90px;">
         <span class="stamp"></span>
         <b>甲方 ： （签字或盖章）</b> 
            <input type="text" style="width: 120px;" name="" id="" readonly="readonly" value="${(travelContractVO.travellers)!''}"/> 
            <span style="margin-left: 20px;">
               <b>乙方 :（签字或盖章）</b> 
               <input type="text" style="width: 120px;" name=""/>
            </span>
         <br/>
         <span><b>日期：</b><input type="text" style="width: 253px;" name="singnDate" id="singnDate" value="${order.createTime?string('yyyy-MM-dd') !''}"/></span>
         <span style="margin-left: 20px;"><b>日期：</b><input type="text" style="width: 253px;" name="lvSingnDate" id="lvSingnDate" value="${order.createTime?string('yyyy-MM-dd') !''}"/></span>
         <br/>
         <br/>
     </p>
     <p>
         ...
         <tr>
            ...
            <p>
              <#if travelContractVO.priceIncludes??>
		            <strong>费用包含：</strong> ${travelContractVO.priceIncludes!''}<br>
	 	      </#if>
              
              <#if travelContractVO.priceNotIncludes??>
		           <strong>费用不包含：</strong> ${travelContractVO.priceNotIncludes!''}<br>
	 	      </#if>
	 	
		      <#if travelContractVO.travelWarnings??>
		           <strong>出行警示及说明：</strong>${travelContractVO.travelWarnings!''}<br>
	 	      </#if>
	 	
		      <#if travelContractVO.travelNotes??>
		           <strong>行前须知：</strong> ${travelContractVO.travelNotes!''} <br>
	 	      </#if>
	 	
		      <#if travelContractVO.backToThat??>
		           <strong>退改说明：</strong> ${travelContractVO.backToThat!''} <br>
	          </#if> 
...
<p align="center">
    <button class="sendBtn"  onclick="editButtomOnclick()">保存且发送</button>
