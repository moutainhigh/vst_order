...
<link href="http://pic.lvmama.com/min/index.php?f=/styles/lv/buttons.css" rel="stylesheet" />
<link href="http://pic.lvmama.com/min/index.php?f=/styles/lv/dialog.css" rel="stylesheet" />
		  ...
		  <table class="e_table form-inline mt10">
                <tbody>
                    ...
                    <#if isTravellerDelay?? && isTravellerDelay>
                     ...
                     <tr>
                        <td colspan="2">
                        <div id="totalOrderPriceDiv2" style="color:red;">
                    ...
                    <tr>
                        <td class="e_label"></td>
                        <td><div class="fl operate"><a id="checkOrderA" class="btn btn_cc1">保存并核对订单</a><a id="closeBookInfoPage" class="btn">取消</a></div></td>
                    </tr>
                </tbody>
            </table>
	</div>
...
<script type="text/javascript">
	 ...
	 var sumbitFlg=false;
     //锁仓前置时判断锁仓是否成功
     var lockSeatFlag = false;
     //锁仓定时查询使用
     var int;
	 nameTxtChange();
	 travellersBindEvent();
	 $(function(){
		$("#totalOrderPriceDiv2").html($("#totalOrderPriceDiv").html());
		setTimeout("$('#orderInfoForm').parents('.dialog').height($('#orderInfoForm').height()+45)",1000);
	});
	
	$('#orderInfoForm').live('click',function(){  
		setTimeout("$('#orderInfoForm').parents('.dialog').height($('#orderInfoForm').height()+45)",500);
	});
	...
	//提交订单
		$('#checkOrderA').bind('click',function(){
				...
				var isPreLockSeat=$("input[name='isPreLockSeat']").val();
				setTimeout("$('#orderInfoForm').parents('.dialog').height($('#orderInfoForm').height()+45)",1000);
				//验证
				...
				$('input[type=text]').each(function(){
					 ...
					 if(requiredTpe || text!=""){
					 	 if(nameType=="fullName"){
					 	 	...
					 	 	if(name.indexOf("travellers")>=0 && subCategoryId =='182' && productType =='INNERLINE'){
					 	 		var veri1 = new RegExp("^[a-zA-Z](?=.*\/)([a-zA-Z]|[/](?!\/)){0,18}[a-zA-Z]$");
					 	 		var veri2 = new RegExp("^[\u4e00-\u9fa5]{1,10}$");
					 	 		if(veri1.test(text)||veri2.test(text)){
					 	 			$(this).next("span").css("display","none");
					 	 		}else{
					 	 			//提示
									$(this).next("span").css("display","");
									alertMsg="请输入正确的姓名，例如张三或zhang/san";
									checkFlag=false;
					 	 		}
					 	 	}else{
        				...
        				if(!myreg.test(text)){
					 	...
					 	if(isPreLockSeat=="true" && isApiFlight() && idType!="ID_CARD"){
							//提示
							$(this).next("span").css("display","");
							alertMsg="必须填写身份证";
							checkFlag=false;
            ...
            //促销优惠劵
            var checkPromotion=$("input[name='choosePromotion']:checked:visible").val();
            if(checkPromotion!=undefined&&checkPromotion!=null&&checkPromotion!=""){
                if(checkPromotion=="promotion"){
                    $(".hiddenCouponList").remove();
                }else if(checkPromotion=="coupon"){
                    $(".promotionMap").remove();
                    $(".promotionMap").attr("disabled","disabled")
                }
            }
			
            //机酒锁仓前置
            if(isPreLockSeat=="true" && isApiFlight()){
            	showLockSeatResult();
            }else{
            	$("input[name='isPreLockSeat']").val("");
            	submitLineBackOrder();
            }
...
//锁仓前置弹层
function showLockSeatResult(){
    nova.dialog({
        title:null,
        content:'<img src="http://pic.lvmama.com/img/new_v/ui_scrollLoading/loadingGIF64px.gif" width="64" height="64"/><p>小驴正在努力帮您占位呢，大约需30秒，请耐心等待哦～</p>',
        width:"440",
        height:"136",
        wrapClass:"loadingPop",
        zIndex:4100,
        initCallback:function(){
        	var fA=$("form[name='orderInfoForm']").serializeArray();
        	var fAString="";
        	$.each(fA,function(index,item){
        		if(item.name.indexOf("travellers")>-1){
        			fAString+="&"+item.name+"="+item.value
        		}
        	})
        	
        	$.each(fA,function(index,item){
        		if(item.name.indexOf("contact")>-1){
        			fAString+="&"+item.name+"="+item.value
        		}
        	})
        	
        	$.each(fA,function(index,item){
        		if(item.name.indexOf("isTestOrder")>-1){
        			fAString+="&"+item.name+"="+item.value
        		}
        	})        	
        	
        	var $form = $("form[name='orderForm']").serialize();
            $form = $form + fAString;
            $.post("/vst_order/ord/order/preLockSeat.do",$form,function(data){
                if(data.success&&data.returnContent!=null){
                    //返回的订单id和子订单id数据放入填单页
                    var lockSetOrderId = data.returnContent;
                    $("input[name='lockSetOrderId']").val(lockSetOrderId);
                    setTimeout(function(){
                        $('.loadingPop .nova-dialog-close').click();
                    },130000);
                    int = setInterval("queryLockSeatResult()",5000);
                }else if(data.success&&data.returnContent==null){
                	$('.loadingPop .nova-dialog-close').click();
                	submitLineBackOrder();
                }else{
                    //锁仓失败
                    nova.dialog({
                        title:null,
                        content:'<span class="order-fail-img"></span><p>非常抱歉，该航班已售罄 </p><a class="btn btn-orange js-to-select">重新选择</a>',
                        dth:"440",
                        height:"208",
                        wrapClass:"failPop",
                        zIndex:4100,
                        initCallback:function(){
                            $('.failPop .nova-dialog-close').on("click", function(){
                            	$('.loadingPop .nova-dialog-close').click();
                            	// 关闭游玩人对话框
                                $('.dialog-blue .dialog-close').click();
                            });
                            var self = this;
                            var $select = self.wrap.find(".js-to-select");
                            $select.on('click',function(){
                                $('.failPop .nova-dialog-close').click();
                                $('.loadingPop .nova-dialog-close').click();
                             	// 关闭游玩人对话框
                                $('.dialog-blue .dialog-close').click();                                
                            });
                    ...
                    //$('#verifiedFlightInfoFailDialog').show();
                }
            },"JSON");
        },
...
//查询锁仓结果
function queryLockSeatResult(){
    $.post("/vst_order/ord/order/queryLockSeatResult.do",{"lockSetOrderId":$("input[name='lockSetOrderId']").val()},function(data){
        if(data.success){
            lockSeatFlag = false;
            window.clearInterval(int);
...
function submitLineBackOrder(){
	var loading	= pandora.loading("正在创建订单...");
	var fA=$("form[name='orderInfoForm']").serializeArray();
	var fAString="";
	$.each(fA,function(index,item){
		if(item.name.indexOf("travellers")>-1){
	...
	formData = formData + fAString;
	$.post("/vst_order/ord/order/lineBackCreateNewOrderResponseBody.do", formData,function(data){
		if(data==null||!data.success){
			...
			loading.close();
			 ...
			 //保存订单游玩人信息
			  $("form[name='orderInfoForm']").find("#orderId").val(data.attributes.orderId);
			  $("form[name='orderInfoForm']").find("#userId").val(data.attributes.userId);
			  $.post("/vst_order/ord/book/saveNewOrderPerson.do",$("form[name='orderInfoForm']").serialize(),function(data){
					loading.close();
					if(data==null||!data.success){
						$.alert(data.message);
						sumbitFlg=false;
