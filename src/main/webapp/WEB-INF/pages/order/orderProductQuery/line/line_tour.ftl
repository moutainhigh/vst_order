 <#if groupPackageList?? && groupPackageList?size &gt; 0>
  <div class="hotel_ebk_wrap">
  	<div class="hotel_tab_box">
  		<div class="hotel_tab">
  			<div class="table_t">跟团游</div>
  					<#list groupPackageList as changeProdPackage>
  						<#if changeProdPackage??&&changeProdPackage.prodPackageDetails?size &gt; 0>
	  						...
	  						<#if changeProdPackage.prodPackageDetails?size gt 1>
	  							 <p class="listName"><a href="#" class="fr moreCategoryLineTour" adultNum="${adultNum}" childNum="${childNum}" outProductId="${productId}" packageGroupId="${detail.groupId}" packageProductId="${detail.prodProduct.productId}" packageProductBranchId="${detail.prodProductBranch.productBranchId}">更多跟团游</a>${changeProdPackage.startDay}日到${changeProdPackage.endDay}日行程</p>
	  						<#else>
        						 ...
        						 <td>
        						 	<#assign prodProductBranch=detail.prodProductBranch />
        						 		...
        						 		<table class="tab_nav" width="100%">
        						 		    <tr>
        						 		       <td colspan="3">
        						 		           <a class="pro_tit" href="http://dujia.lvmama.com/group/${detail.prodProduct.productId}" target="_blank">${detail.prodProduct.productName}</a>
        						 		       </td>
        						 		    </tr>
						        			<tr id="firstLineTourShow${detail.groupId}" groupId="${detail.groupId}" productId="${productId}" parentProductId="${prodProductBranch.productId}" productBranchId="${prodProductBranch.productBranchId}" goodsId="${goods.suppGoodsId}" adult="${adultNum}" child="${childNum}" class="table_nav" >
