			...
			<#if prodProduct.muiltDpartureFlag?? && 'Y'==prodProduct.muiltDpartureFlag>
            	...
            	<input id="visitTime" class="input js_youwanTime" type="text" maxlength="10" name="visitTime"
            	 value="${specDate?string("yyyy-MM-dd")}" readonly="readonly" placeholder="请选择游玩日期">
               	 	...
               	 	<input type="hidden" id="hotelNumValue" name="hotelNumValue" value="${hotelNum}">
            	 	...
            	 	&nbsp;&nbsp;${prodLineRoute.routeName}&nbsp;&nbsp;
               	 	...
               	 	<#if saleCopies>
               	 	<span>成人数:${adultNum}人/儿童数:${childNum}人</span>
               	 	<#else>
               	 		<span>单份(成人数:${baseAdultNum}人/儿童数:${baseChildNum}人)</span>
               	 	</#if>
               	 	份数：
               	 		<#if adultSelect?? && adultSelect?length &gt; 0>
	               	 		<select id="hotelNum" name="quantity">
	               	 			${adultSelect}
	                		</select> 
               	 		<#else>
	               	 		<select id="hotelNum">
                	...
                	<span><a href="javascript:;" class="btn btn_ccproductHoteldetail">查询</a></span>
            </div>            
        </div>