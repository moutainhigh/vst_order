		        				...
		        				<#list reTransportBranchList as pb>
		        					<#list pb.suppGoodsList as goods >
			        					...
			        					<tr class="table_nav" adult="" child="" suppGoodsId="${goods.suppGoodsId}">
						        			...
						        			<td width="20%">
			        							成人数<input goodsId="${goods.suppGoodsId}" auditPrice="${goods.suppGoodsBaseTimePrice.auditPriceYuan}" childPrice="${goods.suppGoodsBaseTimePrice.childPriceYuan}" type="text" class="w5 adultNumText" name="itemMap[${goods.suppGoodsId}].adultQuantity" style="text-align:center" value="${goods.fitAdultQuantity}" required=true number=true />
						        				儿童数<input goodsId="${goods.suppGoodsId}" auditPrice="${goods.suppGoodsBaseTimePrice.auditPriceYuan}" childPrice="${goods.suppGoodsBaseTimePrice.childPriceYuan}" type="text" class="w5 childNumText" name="itemMap[${goods.suppGoodsId}].childQuantity" style="text-align:center" value="${goods.fitChildQuantity}" required=true number=true />
						        				<input id="${goods.suppGoodsId}TotalNum" type="hidden" class="w5 numText" name="itemMap[${goods.suppGoodsId}].quantity" style="text-align:center" value="0" required=true number=true />
						        			</td>
