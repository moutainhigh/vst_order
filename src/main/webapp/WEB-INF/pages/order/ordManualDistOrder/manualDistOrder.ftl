...
$("#txt").ajaxComplete(function(){
  $("#txt").css("display","none");
	...
	$("#saveManDistOrderButton").bind("click",function(){
		...
		$.ajax({
		   url : "/vst_order/order/ordManualDistOrder/manualDistOrder.do",
		   ...
		   success : function(result){
		   		if(result.code=="success"){
		   			...
		   			if(groupMember!='')
		   			{
		   				...
		   				mustDistOrderDialog= new xDialog("/vst_order/order/ordManualDistOrder/showMustDistOrder.do",param,{title:"强制分单",width:600});
		   			}else{
		   				$.alert(result.message);
		   			}
