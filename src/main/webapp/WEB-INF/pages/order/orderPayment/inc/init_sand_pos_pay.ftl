...
<div class="iframe-content">   
	<div class="p_box" id="resultDiv">
	 <form id="commPos" name="commPos" action="https://pay.lvmama.com/payment/pay/sandPos.do" method="post">
            ...
            <tbody>
				...
				<tr>
					...
					<td align="left" valign="middle" class="td1">
						<#if sandPosSubChannelList??>
							<select name="posPaymentType" style="height: 27px;width: 185px;">
							    <#list sandPosSubChannelList as p>
								     <option value=${p.key} <#if p_index = 0>selected="selected"</#if>>${p.value}</option>
							    </#list>
							</select>
						<#else>
							<select name="posPaymentType" style="height: 27px;width: 185px;">
								<option value="SAND_POS" selected="selected">银行卡</option>
								<option value="SAND_POS_CASH">现金</option>
							</select>
						</#if>
					</td>
				</tr>
