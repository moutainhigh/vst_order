	                        ...
	                        <#list resultPage.items as result>
	                                <tr>
	                                    <td>
	                                        ...
	                                        <a title="点击查看订单详情" data-tag="orderItemId" href="/vst_order/order/ordCommon/showOrderDetails.do?objectId=${result.objectId}&objectType=${result.objectType}<#if result.auditTypeName=='催支付'>&isReminderPayment=cuizhifu</#if>" target="_blank">
	                                    	...
	                                    	<form action="/vst_order/ord/order/updateOrderItemConfirmStatus.do" method="post">
	                                    		...
	                                    		<table class="e_table form-inline ">
										            <tbody>
		                                    			...
		                                    			<div style="float: left;">
		                                    				<select name="confirmStatus" autocomplete="off" class="w10" style="margin: 0 0 5px;" value="${result.orderMonitorRst.orderItemConfirmStatus!''}">
	                                    ...
	                                    <td width="210">
											...
											<#if result.orderMonitorRst.isSupplierOrderItem !='Y'>
                                                <a class="btn btn_cc1" data-tag="queryHotelInfo"  title="关房">关房</a>
											</#if>
                                            ...
                                            <#if result.orderMonitorRst.categoryType == 'category_hotel' || result.orderMonitorRst.categoryType == 'category_route_hotelcomb'>
                                                <div style="float: left;margin-top: 5px;margin-right: 5px;">
                                                    <#if result.orderMonitorRst.orderItemConfirmStatus=='FULL' || result.orderMonitorRst.orderItemConfirmStatus=='PECULIAR_FULL'>
                                                        <a class="btn btn_cc1" title="发送满房邮件" onclick="javaScript:sendEmail(this, 'FULL')">发送满房邮件</a>
                                                    </#if>
                                                    <#if result.orderMonitorRst.orderItemConfirmStatus=='CHANGE_PRICE'>
                                                        <a class="btn btn_cc1" title="发送变价邮件" onclick="javaScript:sendEmail(this, 'CHANGE_PRICE')">发送变价邮件</a>
                                                    </#if>
                                                </div>
                                            </#if>
