<tr>
	<td  colspan="2">
	 <input type="hidden" id="ordPersonId${number_index}" name="insurers[${number_index}].ordPersonId" value="${tavellerList[number_index].ordPersonId}"/>
     <table class="e_table" name="userInfoTb" index="${number_index}">
		<tbody>
			<tr>
				 <td class="e_label w12"><b>保险人${number}</b>：</td>
				<td>
					<#if personList??>
					常用联系人：
                	<#list personList as person>
                    	<label class="checkbox mr10">
                    	<input class="checkbox" type="checkbox" name="insurers[${number_index}].receiverId" value="${person.receiverId}" personName="${person.fullName}"
			 ...
			 <tr>
                ...
                <td>
					<input type="text" id="fullName${number_index}" name_type="fullName" name="insurers[${number_index}].fullName" 
					 placeholder="姓名" onchange="" travellersId="" value="${tavellerList[number_index].fullName}" index="${number_index}" required="true"/>
            ...
            <#if number==1>
            	<#if orderRequiredvO.ennameType=="TRAV_NUM_ONE" || orderRequiredvO.ennameType=="TRAV_NUM_ALL">
            	  <tr>
		        	...
		        	<td>
		        		<input class="input js_yz" id="firstName${number_index}" name="insurers[${number_index}].firstName" value="${tavellerList[number_index].firstName}" name_type="firstName" type="text" placeholder="firstName" <#if orderRequiredvO.expandMap['ennameType_OPTIONAL']!='Y'>required="true"</#if>>
		        		<span class="error_text" style="display:none;"><i class="tip-icon tip-icon-error"></i>firstName不正确</span>
		        		<input class="input js_yz" id="lastName${number_index}" name="insurers[${number_index}].lastName"  name_type="lastName" type="text" placeholder="lastName" value="${tavellerList[number_index].lastName}" <#if orderRequiredvO.expandMap['ennameType_OPTIONAL']!='Y'>required="true"</#if>>
		        		<span class="error_text" style="display:none;"><i class="tip-icon tip-icon-error"></i>lastName不正确</span>
		        	</td>
		          </tr>
		         <#else>
		         	<input type="hidden" id="firstName${number_index}" name="insurers[${number_index}].firstName" value="${tavellerList[number_index].firstName}" name_type="firstName" />
		         	<input type="hidden" id="lastName${number_index}" name="insurers[${number_index}].lastName"  value="${tavellerList[number_index].lastName}" name_type="lastName" />
		    	</#if>
		    	<#if orderRequiredvO.occupType=="TRAV_NUM_ONE" || orderRequiredvO.occupType=="TRAV_NUM_ALL">
            	  <tr>
		        	...
		        	<td>
		        		 <select class="select" name="insurers[${number_index}].peopleType" value="${tavellerList[number_index].peopleType}" id="peopleType${number_index}" >
		         ...
		         <#else>
		        	<input type="hidden" name="insurers[${number_index}].peopleType" value="${tavellerList[number_index].peopleType}" id="peopleType${number_index}" />
		    	</#if>
            	 <#if orderRequiredvO.phoneType=="TRAV_NUM_ONE" || orderRequiredvO.phoneType=="TRAV_NUM_ALL">
            	  <tr>
		        	...
		        	<td>
		        		<input id="mobile${number_index}" name="insurers[${number_index}].mobile" value="${tavellerList[number_index].mobile}" id="mobile${number_index}"
		        		 maxlength="11" type="text" placeholder="手机号码" name_type="mobile" <#if orderRequiredvO.expandMap['phoneType_OPTIONAL']!='Y'>required="true"</#if> >
		         ...
		         <#else>
		         	<input  type="hidden" id="mobile${number_index}" name="insurers[${number_index}].mobile" value="${tavellerList[number_index].mobile}"/>
		    	</#if>
		    	...
		    	<#if orderRequiredvO.outboundPhoneType=="TRAV_NUM_ONE" || orderRequiredvO.outboundPhoneType=="TRAV_NUM_ALL">
			    	 <tr>
			        	...
			        	<td>
			        		<input id="outboundPhone${number_index}" name="insurers[${number_index}].outboundPhone" value="${tavellerList[number_index].outboundPhone}" id="outboundPhone${number_index}"
		    	...
		    	<#if orderRequiredvO.emailType=="TRAV_NUM_ONE" || orderRequiredvO.emailType=="TRAV_NUM_ALL">
		        	...
		        	<td>
		        		<input type_name="email" id="email${number_index}" name="insurers[${number_index}].email" value="${tavellerList[number_index].email}" type="text" 
		        			placeholder="邮箱" name_type="email" <#if orderRequiredvO.expandMap['emailType_OPTIONAL']!='Y'>required="true" </#if>>
		       	...
		       	<#else>
		       		<input type_name="email" id="email${number_index}" name="insurers[${number_index}].email" value="${tavellerList[number_index].email}" type="hidden" name_type="email" />
			    </#if>
			    <#if orderRequiredvO.idNumType=="TRAV_NUM_ONE" || orderRequiredvO.idNumType=="TRAV_NUM_ALL">
			    	<tr>
						...
						<td>
							...
							<input type_name="shenfenzheng" id="idNo${number_index}" name="insurers[${number_index}].idNo" value="${tavellerList[number_index].idNo}" type="text" placeholder="证件号码" maxlength="25" name_type="idNo" <#if orderRequiredvO.expandMap['idNumType_OPTIONAL']!='Y'>required="true"</#if>>
			       ...
			       <tr id="person_issued_${number_index}" <#if tavellerList[number_index].idType!="HUIXIANG" && tavellerList[number_index].idType!="TAIBAOZHENG">style="display:none"</#if> >
				       ...
				       <input type='text' id='expDate${number_index}'  name='insurers[${number_index}].expDate' style='width:120px'  readonly='true' onfocus="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd'})" value="<#if tavellerList[number_index].expDate??>${tavellerList[number_index].expDate?string("yyyy-MM-dd")}</#if>"  <#if orderRequiredvO.expandMap['idNumType_OPTIONAL']!='Y'>required="true"</#if>/>

				       	签发地：
				        <input type='text' id='issued${number_index}' name='insurers[${number_index}].issued' style='width:120px'  value="<#if tavellerList[number_index].issued??>${tavellerList[number_index].issued}</#if>"/>
				        </td>	
			        </tr>
			        <tr id="person_birthday_${number_index}" <#if tavellerList[number_index].idType=="ID_CARD" || tavellerList[number_index].idType=="CUSTOMER_SERVICE_ADVICE"||tavellerList[number_index].idType==null>style="display:none"</#if> >
				        ...
				        <td class="data"  <#if tavellerList[number_index].idType!="ID_CARD">style=""</#if>>
				        	...
				        	出生日期<input type='text' id='birthday${number_index}' name='insurers[${number_index}].birthday' style='width:120px' <#if orderRequiredvO.expandMap['idNumType_OPTIONAL']!='Y'>required="true"</#if> readonly='true' onfocus="WdatePicker({readOnly:true,dateFmt:'yyyy-MM-dd'})" value="<#if tavellerList[number_index].birthday??>${tavellerList[number_index].birthday?string("yyyy-MM-dd")}</#if>" <#if orderRequiredvO.expandMap['idNumType_OPTIONAL']!='Y'>required="true"</#if>/>
				        	性别<select name='insurers[${number_index}].gender' id='gender${number_index}' style='width:100px' required="true">
			    ...
			    <#else>
			    	<#-- 证件类型 -->
			    	<input type="hidden" name="insurers[${number_index}].idType" value="${tavellerList[number_index].idType}" />
			    	<#-- 证件号码 -->
			    	<input type="hidden" name="insurers[${number_index}].idNo" value="${tavellerList[number_index].idNo}" />
			    	<#-- 有效期 -->
			    	<input type='hidden' id='expDate${number_index}'  name='insurers[${number_index}].expDate' value="<#if tavellerList[number_index].expDate??>${tavellerList[number_index].expDate?string("yyyy-MM-dd")}</#if>"/>
			    	<#-- 签发地 -->
				     <input type='hidden' id='issued${number_index}' name='insurers[${number_index}].issued' value="<#if tavellerList[number_index].issued??>${tavellerList[number_index].issued}</#if>"/>
				     <#-- 出生日期 -->
				     <input type='hidden' id='birthday${number_index}' name='insurers[${number_index}].birthday' value="<#if tavellerList[number_index].birthday??>${tavellerList[number_index].birthday?string("yyyy-MM-dd")}</#if>"/>
				     <#-- 性别 -->
				    <input type='hidden' name='insurers[${number_index}].gender' value = "${tavellerList[number_index].gender}" id='gender${number_index}' />
			    </#if>
            </#if>
            <#if number!=1>
		    	...
		    	<#if orderRequiredvO.occupType=="TRAV_NUM_ALL">
            	  <tr>
		        	<td class="e_label td_top"><span class="red">*</span>人群：</td>
		        	<td>
		        		<select class="select" name="insurers[${number_index}].peopleType" id="peopleType${number_index}" >
						       	<option value="PEOPLE_TYPE_ADULT" <#if tavellerList[number_index].peopleType=='PEOPLE_TYPE_ADULT'>selected</#if>>成人</option>
					       		<option value="PEOPLE_TYPE_CHILD" <#if tavellerList[number_index].peopleType=='PEOPLE_TYPE_CHILD'>selected</#if>>儿童</option>
			           	</select>
		        	</td>
		        ...
		        <#else>
		        	<input type="hidden" name="insurers[${number_index}].peopleType" id="peopleType${number_index}" value="${tavellerList[number_index].peopleType}" />
		    	</#if>
		        ...
		        <#else>
		        	<input  type="hidden" id="mobile${number_index}" name="insurers[${number_index}].mobile" name_type="mobile" value="${tavellerList[number_index].mobile}"/>
		    	</#if>
