	...
	<tbody>
	  <tr>
         ...
         <tr>
            ...
            <td >
                <div style="width: 345px;">
                    <textarea class="w37" name="remark" maxlength="100"><#if remark??>${remark}</#if></textarea>
        ...
        <tr>
            ...
            <td>
            	<div style="width: 345px;">
                    <textarea class="w37" name="faxMemo"  maxlength="100"><#if remark??>${faxMemo}</#if></textarea>
                    <div class="cc3 tr"><span class="cc1">0</span>/100字（中文(50个)/英文(100个)）</div>
                </div>
            </td>
        </tr>
        ...
        <tr>
        	<#-- 一键下单上下车信息选择，依赖于字符串的拼接  -->
	        <div id="busdata" 
	           	<#if frontBusStop??>data-frontBusStop="${frontBusStop?html}"</#if> 
	           	<#if backBusStop??>data-backBusStop="${backBusStop?html}"</#if>
	        />
	        <#include "/base/bus_stop.ftl"/>
            			...
            			<#if traffic['trafficMap']['toBuses']??&&traffic['trafficMap']['toBuses']?size&gt;0>
					        <div class="no_bd pt_10">
					        	<dl class="user_dl name_check">
					                ...
					                <dd>
					                	<label class="check"><input class="radio" name="additionMap[frontBusStop]" value="<@getBusStopValueStr address="${tobus['address']}" startTime="${tobus['startTime']}" memo="${tobus['memo']}" />" <#if bus_index==0>checked="checked"</#if> type="radio">${tobus['address']}</label><span class="ts_text">发车时间：${tobus['startTime']}</span>
            			...
            			<#if traffic['trafficMap']['backBuses']??&&traffic['trafficMap']['backBuses']?size&gt;0>
					        <div class="no_bd pt_10">
					        	<dl class="user_dl name_check">
					                ...
					                <dd>
					                	<label class="check"><input class="radio" name="additionMap[backBusStop]" value="<@getBusStopValueStr address="${backbus['address']}" startTime="${backbus['startTime']}" memo="${backbus['memo']}" />" <#if bus_index==0>checked="checked"</#if> type="radio">${backbus['address']}</label><span class="ts_text">发车时间：${backbus['startTime']}</span>
...
<script>
	...
	function loadBusStop(){
		$("input[name='additionMap[frontBusStop]'").each(function(){
			if($(this).val()==$("#busdata").attr("data-frontBusStop")){
				$(this).attr("checked","checked");
			}
		});
		$("input[name='additionMap[backBusStop]'").each(function(){
			if($(this).val()==$("#busdata").attr("data-backBusStop")){
				$(this).attr("checked","checked");
			}
		});
	}
