...
<p align="center">
	<button class="pbtn pbtn-small btn-ok" style="margin-top:20px;" id="saveButton">上传</button>
	&nbsp;&nbsp;
	<button class="pbtn pbtn-small btn-ok" style="margin-top:20px;" id="uploadAndSendButton">上传并立即发送</button>
	
	<#if SingleSelection==true && categoryCruise==false>
		&nbsp;&nbsp;
		<button class="pbtn pbtn-small btn-ok" style="margin-top:20px;" id="mailButton">使用邮件模板</button>
	 </#if>
	 
	&nbsp;&nbsp;
	<button class="pbtn pbtn-small btn-ok" style="margin-top:20px;" id="smsButton">使用短信发送</button>
	&nbsp;&nbsp;
...
<script type="text/javascript">
	...
	$("#saveButton").bind("click",function(){
			...
			//异步调用后台
			$.ajax({
			   url : "/vst_order/order/orderShipManage/addOrderNoticeRegiment.do",
	...
	$("#uploadAndSendButton").bind("click",function(){
			//获取页面参数
			var orderIds = $("#orderIds").val();
			var fileId = $("#fileId").val();
			var fileName = $("#fileName").val();
			...
			//异步调用后台
			$.ajax({
			   url : "/vst_order/order/orderShipManage/uploadAndSendNoticeRegiment.do",
			   data : {orderIds:orderIds,fileId:fileId,fileName:fileName,memo:memo},
			   type:"POST",
			   dataType:"JSON",
			   success : function(data){
	...
	var mailButtonDialog;
	$("#mailButton").bind("click",function(){
	
	
		var orderIds = $("#orderIds").val();
		var orderId=orderIds.split("_")[0];
		var email=orderIds.split("_")[1];
		
		window.open("/vst_order/order/orderShipManage/showNoticeRegimentEmailTemplate.do?orderId=" + orderId);
		
		//mailButtonDialog = new xDialog("/vst_order/order/orderShipManage/showUploadNoticeRegiment.do",{"orderId":orderId},{title:"出团通知短信",width:500,height:350});
	});  
	var smsButtonDialog;
	$("#smsButton").bind("click",function(){
		...
		var orderId=orderIds.split("_")[0];
		var email=orderIds.split("_")[1];
		smsButtonDialog = new xDialog("/vst_order/order/orderShipManage/showSendSMSNoticeRegiment.do",{"orderId":orderId},{title:"出团通知短信",width:500,height:350});
	});  
	//取消按钮事件
